  ...
  describe('should preregister Expressway cluster', function () {
    ...
    it('should provision management and calendar connectors', function () {
      ...
      $httpBackend
        ...
        .respond(204, '');
      ClusterService.provisionConnector("clusterId", "c_mgmt");
      $httpBackend.flush();

      $httpBackend
        .expectPOST('http://elg.no/organizations/orgId/clusters/clusterId/provisioning/actions/add/invoke?connectorType=c_cal')
        .respond(204, '');
      ClusterService.provisionConnector("clusterId", "c_cal");
      $httpBackend.flush();
    });
    ...
    it('should add the new cluster to the FMS allow-list', function () {
      ...
      ClusterService.addPreregisteredClusterToAllowList('ew.ree.online', 3600, 'f635d90f-d39b-4659-a983-cf13ca52a960');
      $httpBackend.flush();
