...
class MeetingDetails implements ng.IComponentController {
  ...
  private qualityLabels = ['Good', 'Fair', 'Poor', 'N/A'];
  ...
  private getParticipants(): void {
    this.PartnerSearchService.getUniqueParticipants(this.conferenceID)
      .then((res: IUniqueParticipant[]) => {
        ...
        const lines: IParticipant[] = _.map(res, (item) => this.formateLine(_.get(item, 'participants')));
        this.dataSet = { lines: lines, endTime: _.get(wom, 'endTime'), startTime: _.get(wom, 'startTime'), offset: this.PartnerSearchService.getOffset(timeZone) };
  ...
  private formateLine(lines: any) {//TODO use better type
    const wom = this.PartnerSearchService.getStorage('webexOneMeeting');
  ...
  private getFilterIds(res: object, type: string = 'pstn'): string {
    const arr = type === 'pstn' ?
