...
class TimeLine implements ng.IComponentController {
  ...
  private yAxis(): void {
    ...
    this.yPanel.selectAll('.yaxis')
      ...
      .on('mouseover', (item: IParticipant, index) => {
        ...
        msgArr.push({ key: this.$translate.instant('webexReports.duration'), value: this.PartnerSearchService.toMinOrSec(duration * 1000) });
  ...
  private updateStartPoints(data: Object[]): void {
    _.forEach(data, (item: any) => {//TODO use better type
      let jmtQuality = '';
  ...
  private makeTips(msg: any, top: number, left: number) {//TODO use better type
    let template: string = '';
