...
export class CustomerSearchService {
  ...
  public getMeetings(data: { endDate: string, email: string, startDate: string, meetingNumber: string }): ng.IPromise<IMeetingDetail[]> {
    const url = this.getRequestUrl('v3/meetings');
  ...
  public getMeetingDetail(conferenceID: string): ng.IPromise<IMeeting> {
    const url = this.getRequestUrl(`v3/meetings/${conferenceID}/meeting-detail`);
  ...
  public getUniqueParticipants(conferenceID: string): ng.IPromise<IUniqueParticipant[]> {
    const url = this.getRequestUrl(`v3/meetings/${conferenceID}/unique-participants`);
  ...
  public getParticipants(conferenceID: string): ng.IPromise<IParticipant[]> {
    const url = this.getRequestUrl(`v3/meetings/${conferenceID}/participants`);
  ...
  public getVoipSessionDetail(conferenceID: string, nodeID: string): ng.IPromise<ISessionDetail> {
    const url = this.getRequestUrl(`v2/meetings/${conferenceID}/voip-session-detail`);
  ...
  public getVideoSessionDetail(conferenceID: string, nodeID: string): ng.IPromise<ISessionDetail> {
    const url = this.getRequestUrl(`v2/meetings/${conferenceID}/video-session-detail`);
    return this.$http.post<ISessionDetail>(url, { nodeIds: nodeID }).then(this.WebexReportsUtilService.extractData);
  }

  public getPSTNSessionDetail(conferenceID: string, nodeID: string): ng.IPromise<ISessionDetail> {
    const url = this.getRequestUrl(`v2/meetings/${conferenceID}/pstn-session-detail`);
    return this.$http.post<ISessionDetail>(url, { nodeIds: nodeID }).then(this.WebexReportsUtilService.extractData);
  }

  public getCMRSessionDetail(conferenceID: string, nodeID: string): ng.IPromise<ISessionDetail> {
    const url = this.getRequestUrl(`v2/meetings/${conferenceID}/cmr-session-detail`);
    return this.$http.post<ISessionDetail>(url, { nodeIds: nodeID }).then(this.WebexReportsUtilService.extractData);
  }

  public getJoinMeetingTime(conferenceID: string): ng.IPromise<IJoinTime[]> {
    const url = this.getRequestUrl(`v2/meetings/${conferenceID}/participants/join-meeting-time`);
  ...
  public getServerTime(): ng.IPromise<IServerTime> {
    const url = this.getRequestUrl('v2/server');
  ...
  public getRealDevice(conferenceID: string, nodeID: string): ng.IPromise<ICallType> {
    const url = this.getRequestUrl(`v2/meetings/${conferenceID}/participants/${nodeID}/device`);
  ...
  public getRequestUrl(api: string): string {
    const targetOrgId = this.getAppropriateOrgId();
    if (!_.isEmpty(targetOrgId)) {
      const delim = (api.indexOf('?') >= 0) ? '&' : '?';
      const queryString = `customer_orgid=${targetOrgId}`;
      return `${this.url}${api}${delim}${queryString}`;
    }
    return `${this.url}${api}`;
  }
