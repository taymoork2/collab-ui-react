describe('Controller: Customer Reports Ctrl', function () {
  ...
  describe('when all featuretoggles return false and there are no webex sites', function () {
    beforeEach(function () {
      ...
      spyOn(this.WebexMetricsService, 'hasClassicEnabled').and.returnValue(this.$q.resolve([]));
    ...
    it('should only display spark and care reports tab', function () {
      expect(this.controller.headerTabs).toEqual([headerTabs[0], headerTabs[1], headerTabs[2], headerTabs[4]]);
    });
  ...
  describe('when all featuretoggles return true and there are webex sites', function () {
    beforeEach(function () {
      ...
      spyOn(this.WebexMetricsService, 'hasClassicEnabled').and.returnValue(this.$q.resolve([false, true]));
      spyOn(this.WebexMetricsService, 'checkWebexAccessiblity').and.returnValue(this.$q.resolve([true]));
    ...
    it('should display all reports tabs', function () {
      expect(this.controller.headerTabs).toEqual([headerTabs[0], propackTabs[0], headerTabs[3], headerTabs[4], propackTabs[1]]);
    });
  });
});
