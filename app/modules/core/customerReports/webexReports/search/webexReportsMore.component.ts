  ...
  private getMeetingDetail() {
    this.SearchService.getMeetingDetail(this.conferenceID)
      .then((res) => {
        ...
        data.overview.startTime_ = moment(data.overview.startTime).format('hh:mm');
        data.overview.startDate = moment(data.overview.startTime).format('MMMM Do, YYYY');
        data.overview.endTime_ = data.overview.endTime ? moment(data.overview.endTime).format('hh:mm') : '';
        data.overview.endDate = data.overview.endTime ? moment(data.overview.endTime).format('MMMM Do, YYYY') : '';
