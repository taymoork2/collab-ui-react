...
describe('Controller: UserOverviewCtrl', function () {
  ...
  describe('helper functions:', function () {
    describe('hasLicense():', function () {
      it('should return true if any license ids on the current user that start with the given offer name are present, false otherwise', function () {
        initController.apply(this);
        _.set(this.controller, 'currentUser.licenseID', [
          'foo-fake-license-1',
          'bar-fake-license-2',
        ]);

        expect(this.controller._helpers.hasLicense('foo')).toBe(true);
        expect(this.controller._helpers.hasLicense('bar')).toBe(true);
        expect(this.controller._helpers.hasLicense('baz')).toBe(false);
      });
    });
  });
});
