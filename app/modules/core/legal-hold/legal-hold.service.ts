...
export class LegalHoldService {
  ...
  public createMatter(orgId: string, matterName: string, matterDescription: string, creationDate: Date, usersUUIDList: string[]): IPromise<Matter> {
    const data = {
      ...
      usersUUIDList: usersUUIDList,                             // ACTIVE or RELEASED
    };

    return this.$http.post(this.getActionUrl(Actions.CREATE), data)
      .then(response => {
        return Matter.matterFromResponseData(data, response.data['caseId']);
      });
  }

  public releaseMatter(matter: Matter, releaseDate: Date) {
    matter.release(releaseDate);
  ...
  public readMatter(orgId: string, caseId: string): IPromise<Matter> {
    return this.$http.get(this.getActionUrl(Actions.READ) + `?orgId=${orgId}&caseId=${caseId}`)
      .then(res => {
        return Matter.matterFromResponseData(res.data);
      });
  }
  ...
  public listMatters(orgId: string): IPromise<{}> {
  ...
  public getUserId(emailAddress: string): ng.IPromise<ICustodian> {
