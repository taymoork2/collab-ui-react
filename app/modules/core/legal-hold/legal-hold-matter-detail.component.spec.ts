...
describe('Component: legalHoldMatterDetail', () => {
  ...
  describe('initial state', () => {
    ...
    it('should show "release" or "add users" or "remove users" only for active matters', function (this: Test) {
      expect(this.view.controller('legalHoldMatterDetail').matter.matterState).toBe(MatterState.ACTIVE);
      ...
      this.view.controller('legalHoldMatterDetail').matter.matterState = MatterState.RELEASED;
      this.$scope.$apply();
      expect(this.view.find('.matter-detail__action__delete')).toHaveLength(1);
    ...
    it('should call back end appropriately for releasing a matter', function (this: Test) {
      expect(this.view.controller('legalHoldMatterDetail').matter.matterState).toBe(MatterState.ACTIVE);
      const promise = this.view.controller('legalHoldMatterDetail').releaseMatter().then(() => {
        expect(this.LegalHoldService.releaseMatter).toHaveBeenCalled();
        expect(this.view.controller('legalHoldMatterDetail').matter.matterState).toBe(MatterState.RELEASED);
      });
      expect(promise).toBeResolved();
    });

    it('should call back end appropriately for deleting a matter', function (this: Test) {
      const promise = this.view.controller('legalHoldMatterDetail').deleteMatter();
      expect(promise).toBeResolved();
    ...
    it('should call back end appropriately  to save the name and description when matter is being edited and SAVED', function (this: Test) {
      ...
      this.view.controller('legalHoldMatterDetail').isEdit = true;
      this.$scope.$digest();
      expect(this.view.find('cs-sp-buttons')[0]).toBeVisible();
      expect(this.view.controller('legalHoldMatterDetail').matterEdit.matterName).toBe(testMatterWithUsers.matterName);
      const nameInput = this.view.find('input[name="matterName"]');
      angular.element(nameInput).val('SomeText').trigger('input');
      this.$scope.$digest();
      expect(this.view.controller('legalHoldMatterDetail').matterEdit.matterName).toBe('SomeText');
      const promise = this.view.controller('legalHoldMatterDetail').saveEdit();
      promise.then(() => {
        expect(this.view.controller('legalHoldMatterDetail').matter.matterName).toBe('SomeText');
        expect(this.view.controller('legalHoldMatterDetail').matterEdit.matterName).toBe('SomeText');
      });
      expect(promise).toBeResolved();
    });
    ...
    it('should revert to the previous state when the edit is CANCELLED', function (this: Test) {
      ...
      angular.element(nameInput).val('SomeText').trigger('input');
      expect(this.view.controller('legalHoldMatterDetail').matterEdit.matterName).toBe('SomeText');
      this.view.controller('legalHoldMatterDetail').cancelEdit();
      expect(this.view.controller('legalHoldMatterDetail').matterEdit.matterName).toBe(testMatterWithUsers.matterName);
      expect(this.view.controller('legalHoldMatterDetail').matter.matterName).toBe(testMatterWithUsers.matterName);
    });
  });
});
